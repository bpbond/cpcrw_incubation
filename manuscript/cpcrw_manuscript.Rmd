...
Carbon dioxide fluxes during the incubation ranged from `r pn(fluxsummary["flux_µgC_gC_day_min", "CO2"], 1)` µg C g C^-1^ day^-1^ (`r pn(fluxsummary["flux_µgC_g_day_min", "CO2"], 1)` µg C g soil^-1^ day^-1^) to a maximum of `r pn(fluxsummary["flux_µgC_gC_day_max", "CO2"], 1)` (`r pn(fluxsummary["flux_µgC_g_day_max", "CO2"], 2)`) µg C g C^-1^ day^-1^, with a mean of `r pn(fluxsummary["flux_µgC_gC_day_mean", "CO2"], 1)` (`r pn(fluxsummary["flux_µgC_g_day_mean", "CO2"], 1)`) µg C g C^-1^ day^-1^ over the 100 days. CH~4~ rates ranged from `r pn(fluxsummary["flux_µgC_gC_day_min", "CH4"]*1000, 2)` ng C g C^-1^ day^-1^ (`r pn(fluxsummary["flux_µgC_g_day_min", "CH4"]*1000, 2)` ng C g soil^-1^ day^-1^) to a maximum of `r pn(fluxsummary["flux_µgC_gC_day_max", "CH4"]*1000, 2)` (`r pn(fluxsummary["flux_µgC_g_day_max", "CH4"]*1000, 3)`) ng C g C^-1^ day^-1^, with a mean of `r pn(fluxsummary["flux_µgC_gC_day_mean", "CH4"]*1000, 2)` (`r pn(fluxsummary["flux_µgC_g_day_mean", "CH4"]*1000, 2)`) ng C g C^-1^ day^-1^.

These means conceal considerable variability over the course of the incubation (**Figures 2 and 3**). In the linear mixed-effects model (AIC = `r pn(AIC(step_co2_lme), 1)`), CO~2~ was positively correlated with incubation chamber temperature, core gravimetric water content, and percent soil N (all P < 0.05, and the latter two P < 0.001; **Table 2**). Temperature sensitivity decreased significantly (`r pclean(co2_temp_time)`) over the course of the incubation, while moisture sensitivity was unaffected by time. Percent C and percent N were highly correlated (r = `r pn(cor(cndata$C_percent, cndata$N_percent), 2)`) for these cores. Because percent N was a slightly stronger predictor, it was retained in the model while percent C was excluded; cf. Colman and Schimel {, 2014 #4172}. The interaction between water content and percent N was also highly significant (`r pclean(summary(step_co2_lme)$tTable["WC_gravimetric:N_percent", "p-value"])`), with high-N cores having little relationship between water content and CO~2~ flux (data not shown).

Methane fluxes were positively correlated with percent N, while water content exhibited significant interactions with percent N and DOC (**Table 3**). The model had little predictive power (AIC = `r pn(AIC(step_ch4_lme), 1)`), and neither temperature nor water content was a significant first-order predictor of CH~4~ fluxes.

Temperature and moisture senstivity was generally invariant over time: 
...
discuss - changes over time (see new reuslts)
...
**Table 1.** Summary of dissolved organic carbon (DOC), percent C, percent N, and CO~2~ and CH~4~ fluxes by treatment (with pre-incubation values shown in the right-hand column). Each column summarizes (mean ± s.d.) a separate group of N=6 cores.
