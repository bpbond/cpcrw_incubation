...
fluxdata_orig %>%
  select(Core, Treatment, Temperature, incday, WC_gravimetric, CO2_flux_µmol_g_s, CH4_flux_µmol_g_s) %>%
...
fluxdata %>%
  ...
  do(mod = lm(flux_µmol_g_s ~ WC_gravimetric, data = .)) ->
  models
...
m_co2 <- lm(flux_µmol_g_s ~ Treatment * Temperature + WC_gravimetric,
